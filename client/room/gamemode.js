...
let Inventory = Room.Inventory.GetContext(BlueTeam);
...
Room.Ui.GetContext(BlueTeam).Hint.Value = '!Пройдите, паркур - первым!';
Room.Ui.GetContext(BlueTeam).MainTimerId.Value = MainTimer.Id;
...
case GameModeStateValue:
 Room.Spawns.GetContext(BlueTeam).Enable = true;
 Room.Spawns.GetContext(BlueTeam).Spawn();

  MainTimer.Restart(GameModeTime);
 break;
case EndOfMatchStateValue:
 Room.Spawns.GetContext(BlueTeam).Enable = false;
 Room.Spawns.GetContext(BlueTeam).Despawn();
